...
class OperaOmniaMcpServer {
  ...
  private setupResourceHandlers() {
    ...
    this.server.setRequestHandler<ReadResourceParams, ReadResourceResponse>(ReadResourceRequestSchema, async (request) => {
      try {
  ...
  private setupToolHandlers() {
    ...
    // Handle tool calls
    this.server.setRequestHandler<CallToolParams, CallToolResponse>(CallToolRequestSchema, async (request) => {
      try {
        ...
        switch (name) {
          ...
          case 'list_datasets': {
            const typedArgs = args as unknown as DatasetListInput;
            return this.handleListDatasets(typedArgs);
          }

          case 'get_dataset': {
            const typedArgs = args as unknown as GetDatasetInput;
            return this.handleGetDataset(typedArgs);
          }

          case 'get_random_item': {
            const typedArgs = args as unknown as GetRandomItemInput;
            return this.handleGetRandomItem(typedArgs);
          }

          case 'get_filtered_items': {
            const typedArgs = args as unknown as GetFilteredItemsInput;
            return this.handleGetFilteredItems(typedArgs);
          }

          case 'combine_datasets': {
            const typedArgs = args as unknown as CombineDatasetsInput;
            return this.handleCombineDatasets(typedArgs);
          }

          case 'generate_content': {
            const typedArgs = args as unknown as GenerateContentInput;
   ...
   */
  private async handleListDatasets(args: DatasetListInput) {
    const { category } = args;
    const datasets = await dataFetcher.getDatasetsInCategory(category);
    return {
      content: [
   ...
   */
  private async handleGetDataset(args: GetDatasetInput) {
   ...
   */
  private async handleGetRandomItem(args: GetRandomItemInput) {
    const { category, dataset } = args;
    const datasetContent = await dataFetcher.getDataset(category, dataset);
   ...
   */
  private async handleGetFilteredItems(args: GetFilteredItemsInput) {
   ...
   */
  private async handleCombineDatasets(args: CombineDatasetsInput) {
   ...
   */
  private async handleGenerateContent(args: GenerateContentInput) {
